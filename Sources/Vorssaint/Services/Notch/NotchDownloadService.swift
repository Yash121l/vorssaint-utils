...
final class NotchDownloadService: ObservableObject {
    ...
    @MainActor private final class ProgressCallbacks {
        ...
        func published(_ progress: Progress, id: UUID) {
            ...
            owner.add(progress, id: id)
        }

        func unpublished(_ id: UUID) {
            guard let owner, owner.generation == generation else { return }
            owner.removeProgress(id)
        }
    }
    ...
    func stop() {
        ...
        subscriber = nil
        if let folder, securityScope { folder.stopAccessingSecurityScopedResource() }
        folder = nil
        securityScope = false
        partials.removeAll()
        finished.removeAll()
    ...
    private func add(_ published: Progress, id: UUID) {
        guard let folder, let publication = NotchDownloadPublication(published, folder: folder),
              progress.count < NotchDownloadSupport.maximumObservedFiles else { return }
        progress[id] = published
        ...
        observations[id] = [
            published.observe(\.fractionCompleted) { _, _ in update() },
            published.observe(\.isPaused) { _, _ in update() },
            published.observe(\.isCancelled) { _, _ in update() },
            published.observe(\.isFinished) { _, _ in update() },
            published.observe(\.fileURL) { _, _ in update() },
            published.observe(\.fileOperationKind) { _, _ in update() },
        ]
        refreshItems()
    }

    private func removeProgress(_ id: UUID) {
        guard let removed = progress.removeValue(forKey: id) else { return }
        observations.removeValue(forKey: id)
        if var publication = publications.removeValue(forKey: id), let folder,
           let item = publication.item(removed, folder: folder), item.completed {
    ...
    private func refreshItems() {
        ...
        var active: [NotchDownloadItem] = []
        var invalid: [UUID] = []
        for (id, value) in progress {
            guard var publication = publications[id], let item = publication.item(value, folder: folder) else {
                invalid.append(id)
                continue
            }
            publications[id] = publication
