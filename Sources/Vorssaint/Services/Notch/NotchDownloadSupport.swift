...
struct NotchDownloadPublication {
    ...
    init?(_ progress: Progress, folder: URL) {
    ...
    mutating func item(_ progress: Progress, folder: URL) -> NotchDownloadItem? {
        guard let current = NotchDownloadSupport.publishedURL(progress, folder: folder) else { return nil }
        if current != url {
            url = current
            initialFile = NotchDownloadSupport.fileSnapshot(at: current)
        }
        let file = progress.isFinished ? NotchDownloadSupport.fileSnapshot(at: current) : nil
        ...
        return NotchDownloadItem(id: current.path, url: current, name: current.lastPathComponent,
            receivedBytes: completed ? file?.bytes : progress.kind == .file ? max(0, progress.completedUnitCount) : nil,
...
enum NotchDownloadSupport {
    ...
    static func publishedURL(_ progress: Progress, folder: URL) -> URL? {
