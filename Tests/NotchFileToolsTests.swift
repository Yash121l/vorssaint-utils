...
enum NotchFileToolsTests {
    ...
    private static func publicationContracts(expect: (Bool, String) -> Void) {
        ...
        do {
            ...
            var publication = NotchDownloadPublication(published, folder: folder)!
            published.completedUnitCount = 5
            expect(publication.item(published, folder: folder)?.fraction == 0.5,
                   "a live Progress contributes its actual fraction inside the selected folder")
            published.completedUnitCount = 10
            let missing = publication.item(published, folder: folder)
            ...
            expect(publication.item(published, folder: folder)?.completed == true,
                   "a newly materialized final file can confirm the publisher's completed download")

            published.completedUnitCount = 0
            publication = NotchDownloadPublication(published, folder: folder)!
            published.completedUnitCount = 10
            expect(publication.item(published, folder: folder)?.completed == false,
            ...
            expect(publication.item(published, folder: folder) == nil,
            ...
            var moved = NotchDownloadPublication(renamed, folder: folder)!
            renamed.fileURL = destination
            renamed.completedUnitCount = 10
            expect(moved.item(renamed, folder: folder)?.completed == false,
            ...
            expect(moved.item(renamed, folder: folder)?.active == true,
            ...
            renamed.completedUnitCount = 10
            expect(moved.item(renamed, folder: folder)?.completed == true,
            ...
            var partialPublication = NotchDownloadPublication(renamed, folder: folder)!
            try Data(repeating: 5, count: 10).write(to: partial)
            renamed.completedUnitCount = 10
            expect(partialPublication.item(renamed, folder: folder)?.completed == false,
